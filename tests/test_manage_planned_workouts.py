...
def test_update_converts_exact_flat_template_to_grouped_without_losing_metadata():
    client = ProviderBoundaryManagementClient({7: remote_grouped_plan(flat=True)})
    ...
    assert steps[0]["workoutSteps"][0]["weightValue"] == 90
    assert saved["author"] == {"name": "manual owner"}
    assert saved["unrelatedMetadata"] == {"keep": True}


def test_update_edits_grouped_load_and_preserves_group_and_metadata():
    client = ProviderBoundaryManagementClient({7: remote_grouped_plan()})
    ...
    assert saved_group["workoutSteps"][0]["weightValue"] == 90
...
def test_duplicate_grouped_workout_accepts_equivalent_readback_units():
    from training_sync.garmin.workout_steps import decode_workout_steps

    class NormalizingClient(ProviderBoundaryManagementClient):
        def get_workout_by_id(self, workout_id):
            payload = super().get_workout_by_id(workout_id)
            if int(workout_id) != 7:
                for group in payload["workoutSegments"][0]["workoutSteps"]:
                    for step in group.get("workoutSteps", [group]):
                        if step.get("weightValue") is not None:
                            step["weightValue"] *= 1000
                            step["weightUnit"] = "gram"
            return payload

    remote = deepcopy(project_planned_workout(grouped_plan()).payload)
    remote["workoutId"] = 7
    client = NormalizingClient({7: remote})
    result = duplicate_workout(client, 7, name="Grouped Copy", authorized=True, journal_path=None)

    assert result.state == "verified"
    assert len(decode_workout_steps(client.workouts[result.workout_id]).steps) == 4
